...
export function EditCard() {
  ...
  const [manualOpen, setManualOpen] = useState(
    () =>
      import.meta.env.DEV &&
      new URLSearchParams(window.location.search).get("demo") === "manual",
  );
    ...
    <section className="flex flex-col gap-3 rounded-3xl bg-surface p-4 shadow-border">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs text-muted">
          Mejorá la redacción, acortá los logros o sumá palabras clave del
          aviso.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          <PencilLine aria-hidden strokeWidth={1.5} className="size-4" />A mano
        </Button>
      </div>

      <p className="text-xs text-muted">
        La IA trabaja solo sobre lo que ya escribiste: no inventa empleos ni
        datos, y vos aprobás cada cambio antes de aplicarlo.
      </p>
